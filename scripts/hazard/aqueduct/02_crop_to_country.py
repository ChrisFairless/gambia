...
out_list = []
client = Client()
# client.online = False  # Set to False if you know you already have the API data saved locally
...
def combine_rps_to_hazard(df, clip, country_iso):
    ...
    print(f'Scenario: {df["scenario"].values[0]}')
    ...
    if len(set(df['model'])) == 1:
        filename = f"{country_iso}_{df['filename_base'].values[0]}_{df['scenario'].values[0]}_{df['model'].values[0]}_{df['year'].values[0]}.hdf5"
    else:
        filename = f"{country_iso}_{df['filename_base'].values[0]}_{df['scenario'].values[0]}_ALL_{df['year'].values[0]}.hdf5"
    haz.write_hdf5(Path(clip_dir, filename))
...
for country in country_list:
    ...
    bbox = (180, 90, -180, -90)
    try:
        # TODO replace this with a simpler bounding box from
        # u_coords.get_country_geometries([cntry_iso]).geometry
        exp = client.get_litpop(country)
        bbox = u_coord.latlon_bounds(exp.latitude, exp.longitude, buffer=0.5)
        if bbox[0] < -180 or bbox[2] > 180:
            raise ValueError(f'Bounding box outside -180:180 longitude range. Adjust the buffer or extend the code to deal with this. Bounds: {bbox}')
    ...
    # Create a hazard object from each model
    _ = aqueduct_metadata.groupby(
        ['filename_base', 'scenario', 'model', 'year']
    ).apply(lambda df: combine_rps_to_hazard(df, clip=clip_polygon, country_iso=country_iso))

    # Create a hazard object from all models
    _ = aqueduct_metadata.groupby(
        ['filename_base', 'scenario', 'year']
    ).apply(lambda df: combine_rps_to_hazard(df, clip=clip_polygon, country_iso=country_iso))



#         haz_layers = Hazard.from_raster(
#             files_intensity=haz_file['local_path'],
#             files_fraction=None,
#             attrs=None,
#             band=[1],  # number of return periods in file
#             haz_type='FL',
#             src_crs=None,
...
#         rast = copy.deepcopy(rast_global)
#         rast = rast.rio.clip(crop_polygon)

#         clip_filename = f'{country_iso}_filepath'
#         rast.write()

#         haz_df = rast.to_pandas()
#         haz_df = pd.melt(haz_df, var_name = 'x', value_name='depth', ignore_index=False)
#         haz_df = haz_df[haz_df['depth'] != rast._FillValue]
...
#         haz_df = gpd.GeoDataFrame(haz_df)
