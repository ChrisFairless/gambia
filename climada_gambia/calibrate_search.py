...
def simulate(impf_dict, parameters, scale_impacts):
    ...
    for rp_level in ["lower", "mid", "upper"]:
        ...
        scale_impf(impf_dict_optimised, params["x_scale"], params["y_scale"], temp_impf_file_path)
...
def run_one_parameter_combo(impf_dict_in, parameters, scale_impacts):
    ...
    scale_impf(
        ...
        out_filepath=temp_impf_file_path
    )
...
def scale_impf(impf_dict, x_scale, y_scale, out_filepath):
    ...
    impf_id = impf_id[0]
    impf = impf_set.get_func(haz_type=impf_dict["hazard_abbr"], fun_id=impf_id)
